...
Under "old repository clone URL" enter: https://github.com/uwescience/PV_Cell
...
`PV_Cell/PV_Cell.py` to be called `smallish/smallish.py`

	git mv PV_Cell smallish
	git mv smallish/PV_Cell.py smallish/smallish.py
	git mv smallish/tests/test_PV_Cell.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `PV_Cell` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `PV_Cell` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `PV_Cell` in them. Type
the following to see where all these files are:

	git grep PV_Cell

You can replace `PV_Cell` for `smallish` quickly with:

	git grep -l 'PV_Cell' | xargs sed -i 's/PV_Cell/smallish/g'
...
Edit all the mentions of `PV_Cell` in the `PV_Cell/__init__.py` file, and in
the `PV_Cell/version.py` file as well.
